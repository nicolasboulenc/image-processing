...
const PATH_MAGNET = 4
const PATH_GRID = 5
const PATH_KEY_2_ID = { straight: PATH_STRAIGHT, arc: PATH_ARC, fluid: PATH_FLUID, magnet: PATH_MAGNET, grid: PATH_GRID}
...
const IS_EDGE = '-ms-scroll-limit' in document.documentElement.style && '-ms-ime-align' in document.documentElement.style && !window.navigator.msPointerEnabled
const IS_TRIDENT = !IS_EDGE && !!document.uniqueID // Future Edge might support `document.uniqueID`.
const IS_GECKO = 'MozAppearance' in document.documentElement.style
// Edge has `window.chrome`, and future Gecko might have that.
const IS_BLINK = !IS_EDGE && !IS_GECKO && !!window.chrome && !!window.CSS
// Some engines support `webkit-*` properties.
const IS_WEBKIT = !IS_EDGE && !IS_TRIDENT && !IS_GECKO && !IS_BLINK && !window.chrome && 'WebkitAppearance' in document.documentElement.style

const SHAPE_GAP = IS_TRIDENT || IS_EDGE ? 0.2 : 0.1
...
const isFinite = Number.isFinite || function(value) { return typeof value === 'number' && window.isFinite(value); }

const requestAnim = window.requestAnimationFrame ||
	window.mozRequestAnimationFrame ||
	window.webkitRequestAnimationFrame ||
	window.msRequestAnimationFrame ||
	function(callback) { setTimeout(callback, this.MSPF); }
...
class Anim {
	...
	step() {
		...
		let next = false
		if (this.requestID) {
			cancelAnim.call(window, this.requestID);
			this.requestID = null;
		}
		...
		// this.tasks.forEach(function(task) {
			...
			frame = task.frames[(task.lastFrame = Math.round(timeLen / this.MSPF))];
		...
		if (next) { this.requestID = requestAnim.call(window, this.step.bind(this)); }
	}
	...
	add(valueCallback, frameCallback, duration, count, timing, reverse, timeRatio) {
		let animId = ++(this.newAnimId)
		let frames, stepX, stepT, nextX, t, point
		...
		if (typeof timing === 'string') { timing = this.FUNC_KEYS[timing]; }
		valueCallback = valueCallback || function() {};

		// Generate `frames` list
		if (duration < this.MSPF) {
			frames = [newFrame(0, 0), newFrame(1, 1)];
		} else {
			stepX = this.MSPF / duration;
			frames = [newFrame(0, 0)];
		...
		if (timeRatio !== false) { this.startTask(task, timeRatio); }
	...
	remove(animId) {
		let iRemove;
		if (this.tasks.some(function(task, i) {
	...
	validTiming(timing) {
		return typeof timing === 'string' ? this.FUNC_KEYS[timing] :
...
function forceReflowApply(props) {
	...
	props.reflowTargets = [];
		...
		Object.keys(EFFECTS).forEach(function(effectName) {
			let effectConf = EFFECTS[effectName]
			let effectStats = effectConf.stats
		...
		const LeaderLineAttachment = (function() {
			/**
			 * @class
